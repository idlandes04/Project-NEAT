...
class UnifiedModel(nn.Module):
    def __init__(self, config: Any):
        ...
        if config.use_mvot_processor:
            ...
            if config.vocab_size > 0 and config.hidden_size > 0 :
    ...
    def forward(
        ...
        if self.config.use_mvot_processor and token_type_ids is not None:
             ...
             if image_token_mask.size(1) == seq_len:
                  ...
                  if final_image_hidden_states.numel() == 0:
                       final_image_hidden_states = None
    ...
    def calculate_loss(
        ...
        loss_device = 'cpu' # Default device
        text_logits = model_outputs.get("logits")
        image_logits = model_outputs.get("image_logits")

        if text_logits is not None:
            loss_device = text_logits.device
        elif image_logits is not None:
            loss_device = image_logits.device
        elif labels is not None: # If only labels are present
             loss_device = labels.device
        # If all are None, loss_device remains 'cpu', which is fine for tensor(0.0)
        ...
        elif labels is not None and text_logits is None: # Log if labels are present but logits aren't
             logger.debug("Cannot compute LM loss: 'logits' missing from model output but 'labels' are present.")
